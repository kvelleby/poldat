...
cshp_gw_modifications <- function(western_sahara = TRUE,
  ...
  if(morocco_protectorate){
    ...
    gw <- gw |> dplyr::mutate(status = dplyr::if_else(status == "N/A", "protectorate", status))
  }

  if(western_sahara){
    gw <- gw |>
      dplyr::mutate(end = dplyr::if_else(country_name == "Morocco" & fid == 441, lubridate::ymd("2019-12-31"), end)) |>
      dplyr::filter(!(gwcode == 600 & start > lubridate::ymd("1975-11-13"))) # Remove subsequent coding (442, 443)

    # Spanish Sahara becomes Western Sahara and is occupied by Morocco
    western_sahara <- gw |> dplyr::filter(gwcode == 609)
  ...
  if(palestine){
    ...
    west_bank_and_gaza <- gw |> dplyr::filter(country_name == "West Bank") # This does not include Gaza territory!
    ...
    gw <- gw |> dplyr::mutate(gwcode = dplyr::if_else(country_name == "West Bank", 699, gwcode))
    gw <- gw |> dplyr::mutate(gwcode = dplyr::if_else(country_name == "Gaza", 699, gwcode))

    gw <- dplyr::bind_rows(gw, west_bank_and_gaza)

    gw <- gw |> dplyr::mutate( end = dplyr::if_else(country_name == "Israel" & fid == 512, lubridate::ymd("2019-12-31"), end)) |>
      dplyr::filter(!(gwcode == 666 & fid > 512)) # Not code Israels occupations as part of Israel
    gw <- gw |> dplyr::mutate( end = dplyr::if_else(country_name == "Egypt" & fid == 492, lubridate::ymd("2019-12-31"), end)) |>
      dplyr::filter(!(gwcode == 651 & start > lubridate::ymd("1967-06-09"))) # Not code Israel as owner of Sinai during occupation
  ...
  if(soviet_25dec){
    ...
    gw <- gw |> dplyr::mutate(end = dplyr::if_else(end == as.Date("1991-12-20"), as.Date("1991-12-25"), end))
    gw <- gw |> dplyr::mutate(start = dplyr::if_else(start == as.Date("1991-12-21"), as.Date("1991-12-26"), start))
  }
...
territorial_dependencies_base <- function(...){
  gw <- do.call(cshp_gw_modifications, list(...))
  ...
  for(i in 1:nrow(gw)){
    obs <- gw[i,]
    res <- gw |> dplyr::filter(((start - obs$end) == 1) | ((obs$start - end) == 1))
    contains <- sf::st_contains(obs, res, sparse = F)
    overlaps <- sf::st_overlaps(obs, res, sparse = F)
    idx <- contains | overlaps
    res <- res[idx,] |> dplyr::filter( (uid != obs$uid) )

    if(nrow(res) > 0){
      earlier <- res |> dplyr::filter(start < obs$start)
      later <- res |> dplyr::filter(start >= obs$end)
...
#' Creates a dependency graph of states that have shared territory
#'
#' See poldat::cshp_gw_modifications for descriptions of parameters.
territorial_dependencies <- function(...){
  do.call(territorial_dependencies_base, list(...)) |> suppressMessages()
}
